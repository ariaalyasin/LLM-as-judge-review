...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PAPER 6: Humans or LLMs as the Judge? A Study on Judgement Bias
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}[standout]
  \hypertarget{paper6}{}
  \Large Paper 6:\\Humans or LLMs as the Judge? A Study on Judgement Bias
\end{frame}
...
\begin{frame}{Deceiving LLM Judges: Method and Insights}
  \begin{itemize}
    \item \textbf{Adversarial Setup}: Weak, biased, or factually flawed answers are augmented with fake citations or rich markdown formats to evaluate if surface styling can bypass evaluation safeguards.
    \item \textbf{Potency Discrepancy}: Fake reference insertions are significantly more effective at deceiving model judges than simple layout decorations, highlighting a severe structural vulnerability to artificial authority.
    \item \textbf{The Proximity Effect}: The success of prompt-based hacks surges dramatically when the true quality gap between the candidate answers shrinks.
    \item \textbf{Turnaround Success}: Simple zero-shot surface adjustments exploit authority and beauty biases effectively, achieving up to a 50\% attack success rate on premium judges.
  \end{itemize}
\end{frame}
...
\end{frame}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%++++++++++++++++%%%%%%%%%%%
% PAPER 2: Limits to Scalable Evaluation at the Frontier
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %%

\begin{frame}[standout]
  \hypertarget{paper2}{}
  \Large Paper 2:\\Limits to Scalable Evaluation at the Frontier:\\LLM as Judge Won't Beat Twice the Data
\end{frame}
...
\begin{frame}{Why Judge Bias Matters}
      \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{images/2/Figure_1.png}
        \caption{High agreement between an LLM judge and ground truth does not guarantee accurate model rankings, motivating debiasing methods.}
      \end{figure}
\end{frame}
...
\begin{frame}{Formal Setup}
  \begin{itemize}
    \item \textbf{Evaluation Components}:
      \begin{itemize}
        ...
        \item $\hat{s}$ = Judge score / proxy score (cheap but biased)
      \end{itemize}
  ...
  % --- Added concise math definitions for presentation ---
  \vspace{0.4em}
  \[
    b(m)=P(s(m)=1)
  \]
  ...
  \begin{itemize}
    ...
    \item $p(m), q(m)$: judge accuracies on positives/negatives
  \end{itemize}
\end{frame}
...
\begin{frame}{Why Agreement Is Not Enough}
  ...
  % --- Added concise math for judge bias and agreement ---
  \vspace{0.4em}
  \[
    JB(m)=E[\tilde{s}(m)-s(m)]
  \]
...
\begin{frame}{Debiasing with Prediction Powered Inference (PPI)}
  ...
  % --- PPI estimator and unbiasedness ---
  \vspace{0.4em}
  \[
    \hat{\theta}_{PP}
    =
    \frac{1}{N}\sum_{i=n+1}^{N+n}\tilde{s}_i
    +
    \frac{1}{n}\sum_{j=1}^{n}
    (s_j-\tilde{s}_j)
  \]
  ...
  \]
  \begin{itemize}
    \item First term: many cheap labels. Second term: bias correction from the small labeled set.
  \end{itemize}
\end{frame}
...
\begin{frame}{Sample Efficiency and Theorem 5}
  \begin{itemize}
    ...
    \item \textbf{Theorem 5}: The maximum possible improvement depends on the \textbf{correlation} between true labels ($s$) and proxy labels ($\hat{s}$).
  ...
  % --- Sample-efficiency upper bound ---
  \vspace{0.4em}
  \[
    \tau \le \frac{1}{1-\rho(s,\tilde{s})^2}
  \]
  \begin{itemize}
    \item $\rho$ is the Pearson correlation between $s$ and $\tilde{s}$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Main Result: Theorem 6 + Corollary 7}
  ...
  % --- Formal math statement of the frontier bound ---
  \vspace{0.6em}
  \[
...
\begin{frame}{Experimental Results}
      \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{images/2/Figure_2.png}
        \caption{MMLU results showing that sample-efficiency gains above 2× are uncommon}
      \end{figure}
      \vspace{-0.3cm}
        \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{images/2/Figure_3.png}
        \caption{MT-Bench results confirming that debiased LLM judges rarely exceed a 2× efficiency improvement.}
      \end{figure}
\end{frame}
...
\begin{frame}{Going Beyond Binary Evaluation}
  ...
  % --- Compact theorem statement for continuous scores ---
  \vspace{0.4em}
  \[
    SO(R(\tilde{s})) \le b \Rightarrow \rho(s,\tilde{s})^2 \le 0.5
  \]
